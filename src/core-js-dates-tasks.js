...
function getTime(date) {
  ...
  return dateArr.map((el) => el.toString().padStart(2, '0')).join(':');
}
...
function isDateInPeriod(date, period) {
  ...
  const result =
    dateTime >= new Date(period.start) && dateTime <= new Date(period.end);
  return result;
}
...
function formatDate(date) {
  ...
  return Intl.DateTimeFormat('en-US', settings).format(new Date(date));
}
...
function getWeekNumberByDate(date) {
  ...
  const lastDay = new Date(year, 0, 0);
  const result = Math.ceil((currentDate - lastDay) / (1000 * 60 * 60 * 24 * 7));
  return result;
}
...
function getQuarter(date) {
  ...
  if (monthNumber <= 5) return 2;
  if (monthNumber <= 8) return 3;
  return 4;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const lastWorkDay = new Date(period.end.split('-').reverse().join('-'));

  const result = [];

  while (currentDay <= lastWorkDay) {
    for (let i = 0; i < countWorkDays && currentDay <= lastWorkDay; i += 1) {
      result.push(
        Intl.DateTimeFormat('en-GB').format(currentDay).split('/').join('-')
      );
